...
class IoCertManagerV1IssuerSpec {
  /// Returns a new [IoCertManagerV1IssuerSpec] instance.
  IoCertManagerV1IssuerSpec({
    this.acme,
    this.ca,
    this.selfSigned,
    this.vault,
  ...
  bool operator ==(Object other) =>
      ...
      other is IoCertManagerV1IssuerSpec &&
          other.acme == acme &&
          other.ca == ca &&
  ...
  @override
  String toString() =>
      'IoCertManagerV1IssuerSpec[acme=$acme, ca=$ca, selfSigned=$selfSigned, vault=$vault, venafi=$venafi]';
  ...
  /// Returns a new [IoCertManagerV1IssuerSpec] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static IoCertManagerV1IssuerSpec? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "IoCertManagerV1IssuerSpec[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "IoCertManagerV1IssuerSpec[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return IoCertManagerV1IssuerSpec(
  ...
  static List<IoCertManagerV1IssuerSpec> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final result = <IoCertManagerV1IssuerSpec>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = IoCertManagerV1IssuerSpec.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, IoCertManagerV1IssuerSpec> mapFromJson(dynamic json) {
    final map = <String, IoCertManagerV1IssuerSpec>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = IoCertManagerV1IssuerSpec.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of IoCertManagerV1IssuerSpec-objects as value to a dart map
  static Map<String, List<IoCertManagerV1IssuerSpec>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final map = <String, List<IoCertManagerV1IssuerSpec>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = IoCertManagerV1IssuerSpec.listFromJson(
          entry.value,
          growable: growable,
        );
      }
