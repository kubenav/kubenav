...
import 'package:kubenav/models/plugins/flux/io_fluxcd_toolkit_helm_v2_helm_release_spec_chart.dart';
import 'package:kubenav/models/plugins/flux/io_fluxcd_toolkit_helm_v2_helm_release_spec_chart_ref.dart';
...
import 'package:kubenav/models/plugins/flux/io_fluxcd_toolkit_helm_v2_helm_release_spec_post_renderers_inner.dart';
...
class IoFluxcdToolkitHelmV2HelmReleaseSpec {
  /// Returns a new [IoFluxcdToolkitHelmV2HelmReleaseSpec] instance.
  IoFluxcdToolkitHelmV2HelmReleaseSpec({
    this.chart,
    this.chartRef,
  ...
  ///
  IoFluxcdToolkitHelmV2HelmReleaseSpecChart? chart;
  ...
  /// Consider adding a "default:" property in the specification file to hide this note.
  ///
  IoFluxcdToolkitHelmV2HelmReleaseSpecChartRef? chartRef;
  ...
  List<IoFluxcdToolkitHelmV2HelmReleaseSpecPostRenderersInner> postRenderers;
  ...
  bool operator ==(Object other) =>
      ...
      other is IoFluxcdToolkitHelmV2HelmReleaseSpec &&
          other.chart == chart &&
  ...
  @override
  String toString() =>
      'IoFluxcdToolkitHelmV2HelmReleaseSpec[chart=$chart, chartRef=$chartRef, dependsOn=$dependsOn, driftDetection=$driftDetection, install=$install, interval=$interval, kubeConfig=$kubeConfig, maxHistory=$maxHistory, persistentClient=$persistentClient, postRenderers=$postRenderers, releaseName=$releaseName, rollback=$rollback, serviceAccountName=$serviceAccountName, storageNamespace=$storageNamespace, suspend=$suspend, targetNamespace=$targetNamespace, test=$test, timeout=$timeout, uninstall=$uninstall, upgrade=$upgrade, values=$values, valuesFrom=$valuesFrom]';
  ...
  /// Returns a new [IoFluxcdToolkitHelmV2HelmReleaseSpec] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static IoFluxcdToolkitHelmV2HelmReleaseSpec? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "IoFluxcdToolkitHelmV2HelmReleaseSpec[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "IoFluxcdToolkitHelmV2HelmReleaseSpec[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return IoFluxcdToolkitHelmV2HelmReleaseSpec(
        chart:
            IoFluxcdToolkitHelmV2HelmReleaseSpecChart.fromJson(json[r'chart']),
        chartRef: IoFluxcdToolkitHelmV2HelmReleaseSpecChartRef.fromJson(
            json[r'chartRef']),
        dependsOn:
  ...
  static List<IoFluxcdToolkitHelmV2HelmReleaseSpec> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final result = <IoFluxcdToolkitHelmV2HelmReleaseSpec>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = IoFluxcdToolkitHelmV2HelmReleaseSpec.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, IoFluxcdToolkitHelmV2HelmReleaseSpec> mapFromJson(
      dynamic json) {
    final map = <String, IoFluxcdToolkitHelmV2HelmReleaseSpec>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value =
            IoFluxcdToolkitHelmV2HelmReleaseSpec.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of IoFluxcdToolkitHelmV2HelmReleaseSpec-objects as value to a dart map
  static Map<String, List<IoFluxcdToolkitHelmV2HelmReleaseSpec>>
      mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final map = <String, List<IoFluxcdToolkitHelmV2HelmReleaseSpec>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = IoFluxcdToolkitHelmV2HelmReleaseSpec.listFromJson(
          entry.value,
          growable: growable,
        );
      }
