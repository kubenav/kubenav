...
class PluginArgo extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(centerTitle: true, title: const Text('Argo')),
      bottomNavigationBar: const AppBottomNavigationBarWidget(),
      floatingActionButton: const AppFloatingActionButtonsWidget(),
      body: SafeArea(
        child: SingleChildScrollView(
          child: Column(
            children: List.generate(argoResourceCategories.length, (index) {
              return Column(
                children: [
                  AppVerticalListSimpleWidget(
                    title: argoResourceCategories[index],
                    items: _buildItems(context, argoResourceCategories[index]),
                  ),
