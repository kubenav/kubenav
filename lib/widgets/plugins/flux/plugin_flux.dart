...
class PluginFlux extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(centerTitle: true, title: const Text('Flux')),
      bottomNavigationBar: const AppBottomNavigationBarWidget(),
      floatingActionButton: const AppFloatingActionButtonsWidget(),
      body: SafeArea(
        child: SingleChildScrollView(
          child: Column(
            children: List.generate(fluxResourceCategories.length, (index) {
              return Column(
                children: [
                  AppVerticalListSimpleWidget(
                    title: fluxResourceCategories[index],
                    items: _buildItems(context, fluxResourceCategories[index]),
                  ),
