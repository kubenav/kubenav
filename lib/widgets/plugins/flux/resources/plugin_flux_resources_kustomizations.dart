...
final Resource fluxResourceKustomization = Resource(
  ...
  decodeListData: (ResourcesListData data) {
    ...
    final items = KustomizationlistKustomizeV1.fromJson(parsed).items;
  ...
  decodeList: (String data) {
    ...
    return KustomizationlistKustomizeV1.fromJson(parsed).items;
  },
              ...
              onTap:
                  item.spec?.sourceRef.kind != null &&
                      kindToFluxResource.containsKey(
                        item.spec?.sourceRef.kind.name,
                      )
                  ? (int index) {
                      final resource =
                          kindToFluxResource[item.spec?.sourceRef.kind.name];
                      ...
                      if (resource != null) {
                        ...
                        showSnackbar(
                          context,
                          'Source',
                          '${item.spec?.sourceRef.kind.name} (${item.spec?.sourceRef.namespace ?? item.metadata?.namespace ?? ''}/${item.spec?.sourceRef.name ?? ''})',
                        );
                      }
                    }
                  : null,
