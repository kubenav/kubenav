...
const Service: React.FunctionComponent<ServiceProps> = ({ item, type }) => {
  return (
    <IonGrid>
      <IonRow>
        <Configuration>
          <Row obj={item} objKey="spec.selector" title="Selectors" value={(value) => Object.keys(value).map((label) => <IonChip key={label} className="unset-chip-height">
            <IonLabel>{label}: {value[label]}</IonLabel>
          </IonChip>)} />
          <Row obj={item} objKey="spec.type" title="Type" />
          <Row obj={item} objKey="spec.ports" title="Ports" value={(ports) => ports.map((port: V1ServicePort, index) => <IonChip key={index} className="unset-chip-height">
        ...
        <Status>
          ...
          <Row obj={item} objKey="spec.externalIPs" title="External IPs" value={(externalIPs) => externalIPs.map((externalIP: string, index) => <IonChip key={index} className="unset-chip-height">
            <IonLabel>{externalIP}</IonLabel>
          </IonChip>)} defaultValue="None" />
        </Status>
      </IonRow>
