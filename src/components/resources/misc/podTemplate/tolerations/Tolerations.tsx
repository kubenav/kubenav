import { IonCardContent, IonCardHeader, IonCardTitle, IonCol, IonItem, IonLabel, IonList } from '@ionic/react';
    ...
    <IonCol sizeXs="12" sizeSm="12" sizeMd="12" sizeLg="6" sizeXl="6">
      <IonCardEqualHeight>
        ...
        <IonCardContent>
          <IonList>
            {tolerations
              .filter((toleration) => !!toleration.key)
              .map((toleration, index) => (
                <IonItem key={index}>
                  <IonLabel class="ion-text-wrap">
                    <h2>
                      Schedule on nodes with {toleration.key}
                      {toleration.effect ? `: ${toleration.effect} taint.` : '.'}{' '}
                      {toleration.tolerationSeconds ? `Evict after ${toleration.tolerationSeconds} seconds.` : ''}
                    </h2>
                  </IonLabel>
                </IonItem>
              ))}
